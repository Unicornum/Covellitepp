...
*  | Constant | ^                          | ^         | std::function<bool(const Lights_t *)> | Функция обратного вызова для формирования списка источников света объекта                                            |
*  | ^        | ^                          | ^         | std::function<bool(const void *)>     | Функция обратного вызова для заполнения [пользовательского](\ref CovelliteApiUserConstantBuffer) буфера              |
...
*  | kind     | Требуется Data (kind)      | Параметры | Тип параметра                                     | Описание параметра                            |
*  |----------|----------------------------|-----------|---------------------------------------------------|-----------------------------------------------|
*  | Vertex   | Buffer (\ref Footnote "3") | mapper    | std::function<bool(const [Vertex::Polyhedron] *)> | Функция обратного вызова для изменения буфера |
*  | Index    | ^                          |           |                                                   |                                               |
*  
* \note
