...
*  параметр \b service в виде std::vector (какие данные нужны конкретным
*  рендерам см. в описании соответствующих компонентов).
...
*  | Direction (deprecated)       | x, y, z                     | float                                 | Вектор направления (из указанной точки в начало координат) в пространстве                                |
*  | Attenuation (deprecated)     | const, linear, exponent     | float                                 | Настройки затухания для точечного источника света                                                        |
*  | Rect (\ref Footnote "1")     | left, top, right, bottom    | int                                   | Границы прямоугольника                                                                                   |
*  | Texture                      | content (\ref Footnote "5") | covellite::api::Buffer_t<uint8_t>     | Бинарные данные текстуры в формате R8G8B8A8                                                              |
*  | ^                            | data (deprecated)           | const uint8_t *                       | Бинарные данные текстуры в формате R8G8B8A8                                                              |
...
*  | Shader                       | content (\ref Footnote "5") | covellite::api::Buffer_t<uint8_t>     | Содержимое текстового файла шейдера в бинарном виде                                                      |
*  | ^                            | data (deprecated)           | const uint8_t *                       | Содержимое текстового файла шейдера в бинарном виде                                                      |
*  | ^                            | count (deprecated)          | size_t                                | Размер буфера данных шейдера в байтах                                                                    |
...
*  | Buffer                       | content (\ref Footnote "5") | covellite::api::Buffer_t<[Vertex]>    | Данные вертексного буфера для отрисовки плоских и объемных объектов (\ref Footnote "2")                  |
*  | ^                            | ^                           | covellite::api::Buffer_t<int>         | Данные индексного буфера                                                                                 |
*  | ^                            | data (deprecated)           | const [Vertex] *                      | Данные вертексного буфера для отрисовки плоских и объемных объектов (\ref Footnote "2")                  |
*  | ^                            | ^                           | const [Vertex::Polygon] *             | Данные вертексного буфера для отрисовки плоских объектов (\ref Footnote "2")                             |
*  | ^                            | ^                           | const [Vertex::Polyhedron] *          | Данные вертексного буфера для отрисовки объемных объектов                                                |
*  | ^                            | ^                           | const int *                           | Данные индексного буфера                                                                                 |
*  | ^                            | count (deprecated)          | size_t                                | Количество элементов в буфере, переданном в \b data                                                      |
...
*  | ^                            | dimension (deprecated)      | int                                   | 2 или 3 (по умолчанию) - размерность отрисовываемого объекта                                             |
*  | Fog (deprecated)             | color                       | uint32_t (0xFFFFFFFF)                 | Цвет тумана в формате ARGB                                                                               |
...
*  будет создана внеэкранная поверхность (клмпонент BkSurface), указанная
...
*  | kind         | Требуется Data (kind) | Параметры | Тип параметра (значение по умолчанию) | Описание параметра                                        |
*  |--------------|-----------------------|-----------|---------------------------------------|-----------------------------------------------------------|
*  | -            | Texture               | width     | int                                   | Ширина внеэкранной поверхности в пикселях (только чтение) |
*  | ^            | ^                     | height    | int                                   | Высота внеэкранной поверхности в пикселях (только чтение) |
*  
* \note
*  - Размеры создаваемой внеэкранной поверхности будут учитывать значения
*  параметров \b scale, \b width и \b height у компонента последней
*  установленной камеры.
*  - Параметры \b width и \b height будут содержать фактические размеры
*  внеэкранной поверхности.
*  
...
*  
* \note
*  - Реализации со статическим конвеером используют единственную текстуру (тип
*  можно не указывать), устанавливая ее в нулевой текстурный слот.
...
*  и соответствует формату данных, получаемых от Rocket/RmlUi.
