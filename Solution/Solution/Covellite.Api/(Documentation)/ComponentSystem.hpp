...
*  | ^                               | destination              | std::string                  | Назначение текстуры (albedo (по умолчанию), metalness, roughness, normal, occlusion)                |
...
*  | AlphaTest (\ref Footnote "5")  |                       | discard   | float         | Отбрасывание значений цвета, у которых значение Alpha-канала меньше или равно указанному |
*  | Sampler                        |                       |           |               |                                                                                          |
...
*   + \b normal  = 3.
*   + \b occlusion  = 4.
*  - Реализации со статическим конвеером используют только текстуру типа
*  \b albedo (устанавливая ее в 0-й текстурный слот) и игнорируют остальные.
*  
* ### Shader
*  
*  Компонент шейдера.
...
*  реализации, которые используют шейдеры, должны делать это при помощи шейдера.
*  
