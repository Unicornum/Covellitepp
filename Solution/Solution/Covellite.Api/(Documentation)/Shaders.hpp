...
*  
* \note
*  Если необходимо использовать другую структуру для передачи информации о
*  пикселе, нужно объявить эту структуру и переопределить Pixel через
*  #define Pixel UserPixel (в этом случае следует убедиться, что вертексный
*  и пиксельный шейдер используют для обмена данными одну и ту же структуру).
*  
