...
void GameScene::Render(const Callback_t & _CbObject)
{
  ...
  {
    // Два списка понадобились после задействования updater'ов через рендеры,
    // т.к. теперь изменение списка объектов на рендеринг происходит прямо
    // во время рендеринга текущего списка.
