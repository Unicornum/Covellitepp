...
*  писать один и тот же код, дающий одинаковый результат независимо от платформы
*  (Windows/Android) и графического API (DirectX/OpenGL), использующий для
*  рендеринга GUI библиотеку [RmlUi](https://github.com/mikke89/RmlUi)
*  (близкий по возможностям аналог CSS).
*  
*  Также фреймворк предоставляет [компонентную систему](\ref CovelliteApiComponentSystemPage)
*  рендеринга 2D и 3D объектов, позволяющую строить сцену из объектов, состоящих
*  из набора элементарных 'кубиков', использующих одни и те же данные (шейдеры,
*  текстуры, буферы различных типов) множество раз без дублирования информации
*  в памяти.
*  
