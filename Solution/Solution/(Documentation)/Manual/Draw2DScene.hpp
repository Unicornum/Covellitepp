...
*  
*  Пример создания рендеров для компонентов State.Sampler и индесного буфера, 
*  которые в дальнейшем будут использоваться для всех объектов:
*  
...
*  в списке сцены.
*  
*  Ортографическа камера определяет систему координат, в которой:
...
*  будут только те объекты, которые находятся в диапазоне координат
*  [-1.0f...1.0f] вдоль оси z, с учетом установленного компонента 
*  масштабирования (если, например, параметр z у Data.Scale будет установлен
*  в 0.0f, то независимо от положения на оси z все объекты будут отрисовываться
*  в порядке вызовов их рендеров).
*  
*  Рендеринг объектов
*  ------------------
*  
...
*  
* \todo
*  - Описание использования шейдеров, предоставляемых фреймворком.
*  
...
*  - State.Sampler (в силу специфики работы OpenGL его нужно устанавливать
*  отдельно для \b каждой текстуры и \b обязательно после нее).
*  - Вертексный буфер, содержащий уникальные для данного объекта данные вершин.
*  - Общий для всех объектов (создан ранее) индесный буфер.
*  - Набор компонентов локальной транформации (смещения объектов относительно
*  начала координат).
